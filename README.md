...
**Status:** Core functionality complete. SOLID refactor applied. Audit log complete.
...
Go to **Audit Log** on the dashboard to see a full history of all actions taken in the system. Filter by entity type or username to narrow down specific activity.
...
- Accessible from the main dashboard via the Audit Log button
- Filter by entity type (Customer, Vehicle, RepairOrder, Mechanic) or by username
...
| `AuditRepository` | Audit log writes and reads |
...
| `AuditService` | Logging actions across all entities |
...
│   └── frmAuditLog.cs              — Audit log viewer
├── Helper/
...
│   ├── AuditService.cs             — Audit logging across all entities
